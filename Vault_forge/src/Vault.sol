...
import "@uniswap/v3-core@1.0.1/contracts/interfaces/IUniswapV3Factory.sol";
import "@uniswap/v3-periphery@1.4.3/contracts/interfaces/INonfungiblePositionManager.sol";
import "@uniswap/v3-periphery@1.4.3/contracts/libraries/PositionKey.sol";
...
// Add transfer and allowance functionality if needed
    ...
    IERC20(ASSET_TOKEN_ADDRESS).approve(_positionManager, type(uint256).max);
    ...
    INonfungiblePositionManager.MintParams memory params = INonfungiblePositionManager.MintParams({
        ...
        tickLower: tickLower,
        tickUpper: tickUpper,
        amount0Desired: address(asset) < pairToken ? amount : pairTokenAmount,
        amount1Desired: address(asset) < pairToken ? pairTokenAmount : amount,
    ...
    function collectFees(uint256 positionId) internal returns (uint256 collected0, uint256 collected1) {
    ...
    function removeLiquidity(uint256 positionId) internal returns (uint256 amount0, uint256 amount1) {
    // Get position info
    ...
    uint256 harvestedAsset = asset.balanceOf(address(this)) - assetBalanceBefore;
    uint256 harvestedPairToken = IERC20(pairToken).balanceOf(address(this)) - pairTokenBalanceBefore;
    ...
    function calculatePairTokenAmount(uint256 assetAmount) internal view returns (uint256) {
    address pool = uniswapFactory.getPool(address(asset), pairToken, poolFee);
    ...
    if (address(asset) < pairToken) {
        ...
        price = (1 << 192) / (uint256(sqrtPriceX96) * uint256(sqrtPriceX96));
    }
    
    // Calculate equivalent amount of pair token
    return (assetAmount * price) / (10 ** uint256(IERC20Metadata(address(asset)).decimals()));
    }
    ...
    function maxDeposit(address receiver) public view returns (uint256 maxAssets) {
        ...
        return deposited >= MAX_DEPOSIT_PER_USER ? 0 : MAX_DEPOSIT_PER_USER - deposited;
    }
    ...
    function calculateTickRange() internal view returns (int24 tickLower, int24 tickUpper) {
    ...
    while (withdrawnAssets < neededAssets && i < positionIds.length) {
        ...
        if (activePositions[positionId]) {
            // Remove liquidity
            (uint256 amount0, uint256 amount1) = removeLiquidity(positionId);
            
            // Calculate how much of the asset we got
            uint256 assetAmount = address(asset) < pairToken ? amount0 : amount1;
            withdrawnAssets += assetAmount;
            
            // Convert the pair token to asset if needed
            uint256 pairTokenAmount = address(asset) < pairToken ? amount1 : amount0;
