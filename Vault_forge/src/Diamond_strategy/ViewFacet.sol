...
contract ViewFacet {
        ...
        returns (bool isInProgress, uint256 requestId, bool isFinalized)
    {
        ...
        isFinalized = requestId > 0
            ? ILidoWithdrawal(ds.lidoWithdrawalAddress).isWithdrawalFinalized(requestId)
            : false;
    }
    ...
    function getUnlockTime(
        ...
        for (uint256 i = 0; i < deposits.length; i++) {
            if (!deposits[i].withdrawn) {
                unlockTimes[index] = deposits[i].timestamp + ds.LOCK_PERIOD;
                index++;
            }
        }
    ...
    function getWithdrawableAmount(address user) external view returns (uint256) {
        ...
        if (deposits.length == 0) {
            bool isLocked = block.timestamp <
                ds.depositTimestamps[user] + ds.LOCK_PERIOD;
            return
                isLocked
                    ? (totalUserBalance * ds.LIQUID_PORTION) / 100
                    : totalUserBalance;
        }
        ...
        for (uint256 i = 0; i < deposits.length; i++) {
            if (!deposits[i].withdrawn) {
                if (block.timestamp >= deposits[i].timestamp + ds.LOCK_PERIOD) {
        ...
        uint256 withdrawableFromUnmatured = (unmaturedValue * ds.LIQUID_PORTION) /
            100;
    ...
    function isUpdateNeeded() external view returns (bool) {
        DiamondStorage.VaultState storage ds = DiamondStorage.getStorage();
        return block.timestamp >= ds.lastDailyUpdate + ds.UPDATE_INTERVAL;
    }
    ...
    function maxWithdraw(address _owner) external view returns (uint256) {
        ...
        if (!hasUnlockedDeposits) {
            return (ds.totalAssets * ds.INSTANT_WITHDRAWAL_LIMIT) / 100;
        }
